import { vitePluginRscCore } from "./plugin-BwI7mYXI.js";
import { generateEncryptionKey, toBase64 } from "./encryption-utils-x_yvX0Ey.js";
import { createRpcServer } from "./rpc-Bpt3YsX6.js";
import { normalizeViteImportAnalysisUrl } from "./vite-utils-DqKiG1Jc.js";
...
import { hasDirective, transformDirectiveProxyExport, transformServerActionServer, transformWrapExport } from "@hiogawa/transforms";
...
const clientReferenceMetaMap = {};
const serverResourcesMetaMap = {};
	...
	return [
			...
			async transform(code) {
				...
				for (const match of code.matchAll(/import\.meta\.viteRsc\.loadModule\(([\s\S]*?)\)/dg)) {
					...
					if (this.environment.mode === "dev" && rscPluginOptions.loadModuleDevProxy) {
						...
						s.prepend(`import * as __vite_rsc_rpc from "@hiogawa/vite-rsc/utils/rpc";`);
		...
		},
		{
			...
			generateBundle(_options, bundle) {
				...
				if (this.environment.name === "client") {
					for (const asset of Object.values(rscBundle)) if (asset.type === "asset") this.emitFile({
						type: "asset",
	...
	return [
		{
			...
			async transform(code, id) {
				...
				if (packageSource) if (this.environment.mode === "dev") {
				...
				const result = transformDirectiveProxyExport_(ast, {
					...
					runtime: (name) => `$$ReactServer.registerClientReference(() => { throw new Error("Unexpectedly client reference export '" + ${JSON.stringify(name)} + "' is called on server") },${JSON.stringify(referenceKey)},${JSON.stringify(name)})`
				});
				if (!result) return;
...
function vitePluginUseServer() {
	return [{
		name: "rsc:use-server",
		async configEnvironment(name, config$1, env) {
			if (name === "rsc" && !env.isPreview) {
				config$1.define ??= {};
				if (!config$1.define["__VITE_RSC_ENCRYPTION_KEY__"]) {
					const encryptionKey = await generateEncryptionKey();
					config$1.define["__VITE_RSC_ENCRYPTION_KEY__"] = JSON.stringify(toBase64(encryptionKey));
				}
			}
		},
		async transform(code, id) {
			...
			if (this.environment.name === "rsc") {
				...
				const { output } = transformServerActionServer_(code, ast, {
					runtime: (value, name) => `$$ReactServer.registerServerReference(${value}, ${JSON.stringify(normalizedId)}, ${JSON.stringify(name)})`,
					rejectNonAsyncFunction: true,
					encode: (value) => `$$ReactServer.encryptActionBoundArgs(${value})`,
					decode: (value) => `await $$ReactServer.decryptActionBoundArgs(${value})`
				});
				...
				const result = transformDirectiveProxyExport_(ast, {
					code,
					runtime: (name$1) => `$$ReactClient.createServerReference(${JSON.stringify(normalizedId + "#" + name$1)},$$ReactClient.callServer, undefined, $$ReactClient.findSourceMapURL, ${JSON.stringify(name$1)})`,
